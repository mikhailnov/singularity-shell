...
При сворачивании в трей выполнение JavaScript замораживается — приложение **не грузит процессор**, а при разворачивании — моментально продолжает работать.

![tray](docs/img-ru/tray.png)

Можно настроить **автозапуск**, причем приложение будет заморожено и не будет грузить процессор, но будет готово к работе в любой момент нажатием на иконку в трее!
В официальном клиента функционал автозапуска [отсутствует](https://singularity-app.ru/backlog/118494/) на всех ОС, а здесь — работает на Linux.
...
- **Фоновое автообновление** раз в сутки проверяет Snap Store, загружает новые
...
**Диагностика → Инструменты разработчика** (или клавиша **F12**) открывают
Chromium DevTools для текущей страницы: Console, Network, Application (service
workers, IndexedDB, cookies), Sources. Работает и в главном окне, и во
всплывающих. Для внешнего браузера: `QTWEBENGINE_REMOTE_DEBUGGING=9222
singularity-shell`, затем открыть `http://localhost:9222`.

### Логирование сетевых запросов

Переменная окружения **`SINGULARITY_SHELL_LOG_REQUESTS=1`** включает детальное
логирование всех сетевых запросов в stderr — поверх рабочего фикса CORS (в
отличие от `--diagnose`):
...
- `[net] <тип> <метод> <URL>` — каждый перехваченный запрос (без HTTP-статуса:
  перехватчик срабатывает до получения ответа);
- `[sg-proxy] <метод> <URL>` и `[sg-proxy] HTTP <статус> (<ok|ошибка>) <URL>` —
  проксированные запросы (например, подписка на календарь iCal) **со статусом**;
- `[sg-proxy] refused non-vendor host…` — отказ прокси-релея (защита от SSRF),
  логируется всегда.

По этим строкам удобно видеть, например, что запрос iCal идёт через `sg://`-релей,
а остальные — напрямую.

### Прочее

- `--diagnose` — отдельный перехватчик только для журналирования запросов (ничего не меняет). Побочный эффект: отключается удаление заголовка `deadline`, без которого облачные вызовы вендора блокируются правилами CORS, поэтому эти вызовы могут падать. Для отладки сети лучше переменная `SINGULARITY_SHELL_LOG_REQUESTS=1` выше.
- `--no-auto-update` — отключить фоновое автообновление.
- `--probe` — дамп состояния страницы (service workers, IndexedDB) через 20 с
  после старта и выход (отладочный помощник).
...
#### 11. Системный трей с заморозкой JavaScript

Полноценный рендерер Chromium, исполняющий JavaScript вендора, потребляет
процессор даже в фоне (таймеры, service workers, опрос синхронизации).
Холодный запуск тоже небыстрый — несколько секунд на загрузку SPA и
перерегистрацию SW.

Решение: системный трей с `QWebEnginePage::LifecycleState::Frozen`.
При закрытии или сворачивании в трей (Ctrl+W) страница замораживается —
исполнение JS останавливается, процессор разгружается до ~0%, память
сохраняется. При восстановлении окна вызывается `LifecycleState::Active` —
страница продолжает работу мгновенно, без перезагрузки.

Флаг `--start-hidden` запускает приложение сразу в трей — страница
загружается в фоне.  Окно показывается и тут же скрывается на следующей
итерации event loop (`QTimer::singleShot(0, hide)`) — WebEngine получает
реальную геометрию экрана (SPA рендерит десктопный layout, а не мобильный),
при этом видимой вспышки на экране нет.  Через 12 секунд страница
замораживается и готова к мгновенному восстановлению.

В GNOME без расширения трея приложение работает в обычном режиме
(закрытие = выход).

#### 12. Интеграция с XDG autostart

Переключатель **Включить/Отключить автозапуск** в меню File и в меню трея
управляет desktop-файлом `~/.config/autostart/singularity-shell.desktop`
согласно спецификации XDG autostart.  При включении, если системный файл
(`/etc/xdg/autostart/singularity-shell.desktop`) отсутствует, приложение
копирует свой встроенный desktop-файл (с `Exec=singularity-shell --start-hidden`)
в пользовательский каталог автозапуска.  При выключении пользовательский
файл удаляется, а если есть системный — переопределяется через `Hidden=true`.

Состояние читается напрямую из файловой системы — отдельный ключ конфигурации
не нужен.
...
### Системный трей с заморозкой JavaScript

Закрытие окна или сворачивание в трей (Ctrl+W) замораживает JavaScript —
процессор разгружается до ~0%. Клик по иконке трея или выбор **Показать**
мгновенно восстанавливает окно — страница в том же состоянии, без
перезагрузки и задержек.
...
| `~/.local/share/singularity-shell/settings.conf` | Состояние UI, масштаб, временные метки автообновления |
